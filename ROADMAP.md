...
**Status: achieved (Phases 52–59).** Run `npm run host:match`, share the printed LAN join URL, and two humans can connect to Drydock Span, aim/shoot/reload, see crosshair + hitmarker + health/weapon/ammo HUD + respawn countdown + scoreboard callsigns + round and match-over banners — all from server-owned state. The two-client harness drives the full connect → shoot → kill → match-over flow end to end.
...
Phase 62: server-authoritative **jump** (Space). A shared, tested vertical-motion step (gravity + jump velocity) runs only on the server; the height rides the existing snapshot `y` field so everyone sees the arc, and the client follows server height (no vertical prediction → no jitter). Jump launches from the ground only; collision stays 2D. The harness presses Space and confirms the height rises then lands (`- jump:` line).

UI Settle roadmap (old-school server menus): a focused sequence to make the front-end read like an early-2000s GoldSrc-era multiplayer shooter — a vertical main menu and a dense, sortable server-browser table with beveled dark "VGUI" chrome. These are genre-wide conventions, not any one game's assets; all names, the wordmark, and the palette stay original to Breachline, and no existing game's art, logo, or pixel layout is reproduced.

- UI-1 — VGUI shell + main menu. Replace the centered card with a beveled dark panel: the Breachline wordmark and a vertical menu (Find Servers, Settings, Controls). A tested pure nav-state helper picks the active panel. Condensed/mono type, beveled borders.
- UI-2 — Server browser table. The Find Servers panel becomes a dense table with sortable column headers (Name / Players / Map / Ping), tabs (Internet / Recent / Favorites), row selection, double-click to join, and an action bar (Refresh / Connect / Add Favorite / Add Server). Pure tab-filter + column-sort + favorites helpers are unit-tested.
- UI-3 — Favorites + history persistence. Favorites and recent history persist in localStorage and back the matching tabs.
- UI-4 — Ping + latency bars. A bounded client-side ws probe measures ping per listed server; show a ping column and latency bars, sortable by ping.
- UI-5 — Settings panel. Mouse sensitivity and FOV (and crosshair) persist and apply to the game.
- UI-6 — Controls panel + final polish. A keybind reference, subtle CRT/scanline treatment, condensed type pass, responsive layout.

Phase 64: optional **match registry + main-menu server browser** (a deliberate, opt-in move past local-only). A self-hostable `apps/registry` HTTP service lists opt-in public matches: hosts publish a validated, bounded announcement, heartbeat under a TTL, and unlist on shutdown; `host:match` publishes only when `BREACHLINE_REGISTRY_URL` is set, so a plain LAN match never broadcasts. `/playtest.html` opens on a server browser that fetches the build-filtered list, merges it with a localStorage recent-servers history, and connects on click or manual entry — hiding once connected. Discovery only: the registry owns the list, never gameplay; the game server stays authoritative. Pure logic (announcement validation, registry store/router, list merge, recent store, manual-URL parse) is unit-tested; a registry smoke and the browser-page smoke cover the service and menu. Public-internet deployment is a later ops step; proven self-hostable locally.

Phase 63: server-authoritative **crouch**, visible to all (Ctrl or C). A `crouch` input button makes the server halve the player's planar speed and mark a crouch stance that broadcasts to everyone via a flag packed into the snapshot entity `active` field (`SNAPSHOT_ENTITY_FLAGS`) — the 28-byte entity stride and protocol shape are unchanged. The stance lowers the shared eye point (`playerEyeHeightMeters`: 1.62 standing / 1.0 crouched) for the hitscan source/target, the local camera, and the remote stand-in, so a crouched player ducks a level shot and crouched models squash. The client mirrors the ×0.5 multiplier in prediction for parity but owns none of it (server owns speed, stance, eye height). A HUD `Stance` readout shows standing/crouched; the harness holds the crouch key and confirms the server-owned stance flips (`- crouch:` line).
...
59. **Two-human local match validation.** ✅ Done. The harness runs its dev server with a frag limit of 1 so its confirmed kill ends the match, then asserts the match-over banner names the winner (a `- match result:` line, honest caveat otherwise). The playtest review documents the real host-a-match flow (host:match → connect → aim/shoot/reload → death/respawn → round and match banners) as the first playable-match proof, with honest local-prototype caveats.
...
- **R6 — Grenade + armor.** ✅ Done. **Armor** is a server-owned damage buffer that absorbs a fraction of each hit until it depletes, bought in the buy menu and reset each round. A buyable **grenade** (one per round, thrown with G) is fully server-simulated — an arc under gravity, clamped to the arena, that detonates on a fuse and deals falloff blast damage through the same armor/death path (friendly-fire-aware; self-damage possible). The client sends only a throw intent; the server broadcasts live grenades for rendering and keeps the held count private. The staged competitive loop is complete: **sides → economy → buy → objective → armor + grenade.**
